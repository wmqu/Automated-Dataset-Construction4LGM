...
class CEPSUM(DataProcessor):
    ...
    pages = "5712--5717",
    abstract = "In this work, we present a affnet_model to generate e-commerce product summaries. The consistency between the generated summary and the product attributes is an essential criterion for the ecommerce product summarization task. To enhance the consistency, first, we encode the product attribute table to guide the process of summary generation. Second, we identify the attribute words from the vocabulary, and we constrain these attribute words can be presented in the summaries only through copying from the source, i.e., the attribute words not in the source cannot be generated. We construct a Chinese e-commerce product summarization dataloader, and the experimental results on this dataloader demonstrate that our models significantly improve the faithfulness.",
    }
    """
    def __init__(self):
        super().__init__()
