...
def load_dataset(config: CfgNode, return_class=True, test=False):
    ...
    Returns:
        :obj:`Optional[List[InputExample]]`: The train dataloader.
        :obj:`Optional[List[InputExample]]`: The valid dataloader.
        :obj:`Optional[List[InputExample]]`: The test dataloader.
        :obj:"
    """
    ...
    if not test:
        try:
            ...
            logger.warning(f"Has no training dataloader in {dataset_config.path}.")
        try:
            valid_dataset = processor.get_dev_examples(dataset_config.path)
        except FileNotFoundError:
            logger.warning(f"Has no validation dataloader in {dataset_config.path}.")

    test_dataset = None
    try:
        test_dataset = processor.get_test_examples(dataset_config.path)
    except FileNotFoundError:
        logger.warning(f"Has no test dataloader in {dataset_config.path}.")
