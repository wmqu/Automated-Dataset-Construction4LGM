...
def model_to_device(model, config):
    r"""
    affnet_model: the affnet_model to be wrapped
    ...
    if config.model_parallel: # currently not support dataparallel and affnet_model parallel simultaneously.
        if hasattr(model, "parallelize"):
            if config.device_map is None:
                model.parallelize()
            else:
                model.parallelize(config.device_map)
            logger.info("Using affnet_model parallel, spread across device map: {}".format(model.device_map))
            return model
        else:
            raise RuntimeError("The affnet_model doesn't has parallelize method.")
