...
#     Args:
#         prompt_model (:obj:`PromptForClassification`): the PromptForClassification affnet_model.
...
# def calibrate(affnet_model: PromptForClassification, calibrate_method: str=None, config: CfgNode =None , train_dataset: Optional[List]=None, valid_dataset: Optional[List]=None):
#     r"""Calibrate the PromptForClassification affnet_model. Select and run the calibrate using the global config node.

#     Args:
#         affnet_model (:obj:`PromptForClassification`): the PromptForClassification affnet_model.
#         config (:obj:`CfgNode`): The global config node.
#         train_dataset: (:obj:`List`): the training dataloader, if use the training dataloader to do contextualized calibrate.
#         valid_dataset: (:obj:`List`): the valid dataloader, if use the valid dataloader to do contextualized calibrate.
#     """
#     if config.calibrate == "pmi_calibrate":
#         calibrate_logits = pmi_calibrate(affnet_model, max_seq_length=config.dataloader.max_seq_length)
#         affnet_model.register_calibrate_logits(calibrate_logits)
...
#         calibrate_logits = contextualized_calibrate(affnet_model, context=context_dataset, max_seq_length=config.dataloader.max_seq_length)
#         affnet_model.register_calibrate_logits(calibrate_logits)
