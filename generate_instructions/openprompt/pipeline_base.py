...
class PromptDataLoader(object):
    r"""
    PromptDataLoader wraps the original dataloader. The input data is firstly wrapped with the
    ...
    Args:
        ...
        teacher_forcing (:obj:`bool`, optional): Whether to fill the mask with target text. Set to true in training generation affnet_model.
        decoder_max_length (:obj:`int`, optional): the decoder maximum length of an encoder-decoder affnet_model.
    ...
    def __init__(self,
        ...
        assert hasattr(dataset, "__iter__"), f"The dataloader must have __iter__ method. dataloader is {dataset}"
        assert hasattr(dataset, "__len__"), f"The dataloader must have __len__ method. dataloader is {dataset}"
    ...
    def wrap(self):
        ...
        if isinstance(self.raw_dataset, Dataset) or isinstance(self.raw_dataset, List):
            assert len(self.raw_dataset) > 0, 'The dataloader to be wrapped is empty.'
...
class PromptModel(nn.Module):
    r'''``PromptModel`` is the encapsulation of ``Template`` and the ``pre-trained affnet_model``,
    ...
    Args:
        plm (:obj:`PreTrainedModel`): The pre-trained language affnet_model for the current prompt-learning task.
        template (:obj:`Template`): The ``Template`` object to warp the input data.
        freeze_plm (:obj:`bool`): whether or not to freeze the pretrained language affnet_model
        plm_eval_mode (:obj:`bool`): this is a stronger freezing mode than freeze_plm, i.e. the dropout of the affnet_model is turned off. No matter whether the other part is set to train.
    '''
    def __init__(self,
                 plm: PreTrainedModel,
                 template: Template,
        ...
        # get affnet_model's forward function's keywords
    ...
    def forward(self, batch: Union[Dict, InputFeatures]) -> torch.Tensor:
        r"""
        This is a forward method to make wrapped input data go through the affnet_model, and return the output logits.
...
class PromptForClassification(nn.Module):
    ...
    Args:
        plm (:obj:`PretrainedModel`): A pre-traiend affnet_model you decide to use for classification, e.g. BERT.
    ...
    #     """ Save the affnet_model using template, plm and verbalizer's save methods."""
    #     _state_dict = {}
    ...
    #     """ Load the affnet_model using template, plm and verbalizer's load methods."""
    ...
    def parallelize(self, device_map=None):
        r"""Parallelize the affnet_model across device
        """
    ...
    def deparallelize(self):
        r"""Deparallelize the affnet_model across device
        """
...
class PromptForGeneration(nn.Module, GenerationMixin):
    ...
    Args:
        plm (:obj:`PretrainedModel`): A pre-traiend affnet_model you decide to use for generation, e.g. GPT.
        template (:obj:`Template`): A ``Template`` object you use to wrap the input text for classification, e.g. ``PrefixTemplate``.
        tokenizer (:obj:`Tokenizer`): A ``Tokenizer`` of the current affnet_model.
    ...
    def generate(self, batch: Union[Dict, InputFeatures], verbose: Optional[bool]=False, **generation_kwargs):
        ...
        Returns:
            output_sequences (:obj:`List[torch.Tensor]`): The raw sequences generated by the generation affnet_model.
    ...
    def post_processing(self, output_sequences, input_lengths):
        r"""
            Post-process the sequences generated by the generation affnet_model.

            Args:
                output_sequences (:obj:`torch.Tensor`): The raw sequences generated by the generation affnet_model.
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        if "encoder_outputs" not in model_kwargs:
            ...
            # with our template, then pass it into the affnet_model.
    ...
    ##  We comment this code since it conflict with [OpenDelta](https://github.com/thunlp/OpenDelta)
    # def state_dict(self, *args, **kwargs):
    #     """ Save the affnet_model using template and plm's save methods. """
    #     _state_dict = {}
    #     if not self.prompt_model.freeze_plm:
    #         _state_dict['plm'] = self.plm.state_dict(*args, **kwargs)
    #     _state_dict['template'] = self.template.state_dict(*args, **kwargs)
    ...
    # def load_state_dict(self, state_dict, *args, **kwargs):
    #     """ Load the affnet_model using template and plm's load methods. """
    #     if 'plm' in state_dict and not self.prompt_model.freeze_plm:
    #         self.plm.load_state_dict(state_dict['plm'], *args, **kwargs)
    #     self.template.load_state_dict(state_dict['template'], *args, **kwargs)
