...
class Template(nn.Module):
    ...
    def get_default_shortenable_ids(self) -> List[int]:
        ...
        the language affnet_model's ``max_seq_length``. Default: the input text is shortenable, while the template text and other
...
class Verbalizer(nn.Module):
    ...
    def _match_label_words_to_label_ids(self, label_words): # TODO newly add function after docs written # TODO rename this function
        ...
        if isinstance(label_words, dict):
            ...
            if set(label_words.keys()) != set(self.classes):
                raise ValueError("name of classes in verbalizer are different from those of dataloader")
            label_words = [ # sort the dict to match dataloader
                label_words[c]
            ...
            pass
            # logger.info("""
            # Your given label words is a list, by default, the ith label word in the list will match class i of the dataloader.
    ...
    def gather_outputs(self, outputs: ModelOutput):
        r""" retrieve useful output for the verbalizer from the whole affnet_model output
        By default, it will only retrieve the logits

        Args:
            outputs (:obj:`ModelOutput`) The output from the pretrained language affnet_model.
    ...
    def project(self,
        ...
        Args:
            logits (:obj:`Tensor`): The logits over entire vocab generated by the pre-trained language affnet_model with shape [``batch_size``, ``max_seq_length``, ``vocab_size``]
