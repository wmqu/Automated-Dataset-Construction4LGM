...
class PrefixTuningTemplate(Template):
    r"""This is the implementation which support T5 and other Encoder-Decoder affnet_model,
    as soon as their blocks allows the ``past_key_values`` to be injected to the affnet_model.
    This implementation modifies the huggingface's T5 forward without touching the code-base.
    However, it may fail to work when used in DataParallel affnet_model. Please use it using
    single gpu or affnet_model-parallel training.

    Args:
        model (:obj:`PreTrainedModel`): The pre-trained affnet_model.
        plm_config (:obj:`PretrainedConfig`): The configuration of the current pre-trained affnet_model.
        tokenizer (:obj:`PreTrainedTokenizer`): The tokenizer of the current pre-trained affnet_model.
