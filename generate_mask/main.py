...
def get_argparser():
    ...
    parser.add_argument("--dataset", type=str, default='umd', choices=['iit', 'umd'],
...
def get_dataset(opts):
    ...
    if opts.dataset.lower() == 'umd':
        train_dst = UMD(root=opts.data_root, split='train', transform=train_transform)
        val_dst = UMD(root=opts.data_root, split='val', transform=val_transform)
    elif opts.dataset.lower() == 'iit':
        train_dst = IIT(root=opts.data_root, split='train', transform=train_transform)
        val_dst = IIT(root=opts.data_root, split='val', transform=val_transform)
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                save_ckpt('checkpoints/%s/latest_%s_%s_os%d.pth' %
                          (opts.dataset, opts.model, opts.dataset, opts.output_stride))
                print("validation...")
                model.eval()
                val_score, ret_samples = validate(
                    opts=opts, model=model, loader=val_loader, device=device, metrics=metrics, ret_samples_ids=vis_sample_id)
